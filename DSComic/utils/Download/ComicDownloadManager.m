...
#import "ComicDownloadManager.h"
#import "NSString+MD5.h"
#import "NSURLSessionTask+Extension.h"

@interface ComicDownloadManager ()<NSURLSessionDataDelegate>
...
@implementation ComicDownloadManager

#pragma mark - Public
+ (instancetype)sharedManager{
    static ComicDownloadManager *instance = nil;
...
-(void)downloadWithUrl:(NSString *)url ComicID:(NSInteger)comicID ChapterID:(NSInteger)chapterID resume:(BOOL)resume progress:(DProgressBlock)progressBlock state:(StateBlock)stateBlock{
    ...
    // 将chapterID值作为key
    NSString *key = [NSString stringWithFormat:@"%ld",chapterID];
...
//    NSLog(@"OY===getPlist path:%@",path);

    NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithContentsOfFile:path];
//    NSLog(@"OY===getPlist dict:%@",dict);
...
//    NSLog(@"OY===getDownloadedPathWithKey");
...
//    NSLog(@"OY===setPlistValue dict:%@",dict);

}

// 删除plist记录
- (void)removePlistValueForKey:(NSString *)key {
//    NSLog(@"OY===removePlistValueForKey");
